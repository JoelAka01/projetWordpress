...
// get a specific icon with attributes
function esgi_getIconWithAttributes($name, $attributes = [])
{
    $icon = esgi_getIcon($name);
    
    if (empty($icon) || empty($attributes)) {
        return $icon;
    }
    ...
    return str_replace('<svg', '<svg' . $attr_string, $icon);
}
